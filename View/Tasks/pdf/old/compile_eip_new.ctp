<?php
    $ucs = $this->Session->read('Auth.User.Compile');
    
Configure::write('debug', 0);
        //$this->layout='/pdf/default';

    $show_threaded = $ucs['show_threaded'];
    $show_details = $ucs['show_details'];
    $color_team = $ucs['color_team'];
    $showteams = $ucs['Teams'];
    $privateteams = $ucs['Private'];
    ...
    function makeIts($tt=array()){
        ...
        if (!empty($lead_team)){
            $buttons.= '<span>'.$lead_team.'</span>';
        }

        if (!empty($linked_parent_team)){
            $buttons.= '&nbsp;&gt;&gt; <span>'.$linked_parent_team.'</span>';
        }
            ...
            <?php foreach ($tasks as $task): ?>
                <tr>
                    ...
                    <?php 
                        ...
                        $buttons = '';
                            if (!empty($tt_l)){
                                $buttons.= '<span>'.$tt_l[0].' </span>&nbsp;';
                            }
                            if (!empty($tt_lp)){
                                $buttons.= '&nbsp;&gt;&gt; <span>'.$tt_lp[0].' </span>&nbsp;';
                            }

                            foreach ($tc as $ct_code){
                                $buttons.= '<span>'.$ct_code.' </span>&nbsp;';
                            }
                            echo $buttons;
