<?php
    ...
    $pgLabels = $this->Session->read('Auth.User.Compile.PageLabels');
    $today = date('Y-m-d');
...
");
    ...
    // Due Date
    elseif($view_type == 2){
        $viewMessage = 'Viewing <b>tasks with due dates</b> involving <b>'.$viewTeamStr.'</b> from <b>ANY</b> date, ordered by <b>ascending due date</b>';
    }
    // Assisting
    elseif($view_type == 3){
        $viewMessage = 'Viewing tasks where <b>'.$viewTeamStr.'</b> are <b>assisting</b> from <b>ANY</b> date, ordered by '.$viewSort.'</b>';
    }
    // Assisting
    elseif($view_type == 4){
        $viewMessage = 'Viewing <b>tasks due within 2 weeks</b> where <b>'.$viewTeamStr.'</b> are <b>assisting</b>, ordered by '.$viewSort.'</b>';
    }
    // Action Items
    elseif($view_type ==  5){
    ...
    // Recently Created
    elseif($view_type ==  6){
        $viewMessage = '<b>Viewing <b>recently created</b> tasks involving <b>'.$viewTeamStr.'</b> from <b>ANY</b> date, ordered by <b>descending created date</b> (i.e newest first)';
    }
}
